...
func TestTrySubstituteOutputDirect(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "nested property access - path",
			input:    "${{ outputs.nested.path }}",
			expected: "file.txt",
			ok:       true,
		},
		{
			name:     "nested property access - bytes",
			input:    "${{ outputs.nested.bytes }}",
			expected: 1234,
			ok:       true,
		},
		{
			name:     "nested property access - content",
			input:    "${{ outputs.nested.content }}",
			expected: "file data",
			ok:       true,
		},
		{
